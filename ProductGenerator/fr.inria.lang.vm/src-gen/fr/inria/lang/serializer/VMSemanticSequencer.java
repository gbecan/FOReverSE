...
public class VMSemanticSequencer extends AbstractDelegatingSemanticSequencer {
	...
	public void createSequence(EObject context, EObject semanticObject) {
		if(semanticObject.eClass().getEPackage() == VMPackage.eINSTANCE) switch(semanticObject.eClass().getClassifierID()) {
			...
			case VMPackage.VARIABILITY_MODEL:
				if(context == grammarAccess.getVariabilityModelRule()) {
					sequence_VariabilityModel(context, (VariabilityModel) semanticObject); 
					return; 
				}
				else break;
	...
	/**
	 * Constraint:
	 *     (notTranslatable?='@NT'? runTime?='@RT'? optional?='?'? ((min=Integer max=Integer) | (min=Integer max=Integer))? name=ID)
	 */
	...
	protected void sequence_ImportDeclaration(EObject context, ImportDeclaration semanticObject) {
		if(errorAcceptor != null) {
			if(transientValues.isValueTransient(semanticObject, VMPackage.Literals.IMPORT_DECLARATION__IMPORT_URI) == ValueTransient.YES)
				errorAcceptor.accept(diagnosticProvider.createFeatureValueMissing(semanticObject, VMPackage.Literals.IMPORT_DECLARATION__IMPORT_URI));
		}
		INodesForEObjectProvider nodes = createNodeProvider(semanticObject);
		SequenceFeeder feeder = createSequencerFeeder(semanticObject, nodes);
		feeder.accept(grammarAccess.getImportDeclarationAccess().getImportURIQUALIFIEDNAMEParserRuleCall_1_0(), semanticObject.getImportURI());
		feeder.finish();
	}
	...
	protected void sequence_PackageDeclaration(EObject context, PackageDeclaration semanticObject) {
		if(errorAcceptor != null) {
			if(transientValues.isValueTransient(semanticObject, VMPackage.Literals.PACKAGE_DECLARATION__NAME) == ValueTransient.YES)
				errorAcceptor.accept(diagnosticProvider.createFeatureValueMissing(semanticObject, VMPackage.Literals.PACKAGE_DECLARATION__NAME));
		}
		INodesForEObjectProvider nodes = createNodeProvider(semanticObject);
		SequenceFeeder feeder = createSequencerFeeder(semanticObject, nodes);
	...
	/**
	 * Constraint:
	 *     (relationships=Relationships attributes=Attributes? descriptions=Descriptions? constraints=Constraints?)
	 */
	protected void sequence_VariabilityModel(EObject context, VariabilityModel semanticObject) {
		genericSequencer.createSequence(context, semanticObject);
	}
